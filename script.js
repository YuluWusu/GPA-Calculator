    ...
    if (useRealAPI) {
        try {
            ...
            if (gradesData.success) {
                console.log("Đã lấy được HTML bảng điểm thật:", gradesData.html);
                alert("Đã kết nối API thật thành công và tải HTML! (Xem console). Cần viết logic parse HTML để đưa vào bảng.");
                btn.innerHTML = originalText;
                btn.disabled = false;
                closeImportModal();
                return;
